...
const SIDEBAR_SCROLL_TOP_KEY = 'sidebar:content:scrollTop';
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(({ className, ...props }, ref) => {
    ...
    React.useEffect(() => {
        ...
        try {
            const saved = window.sessionStorage.getItem(SIDEBAR_SCROLL_TOP_KEY);
            if (saved) {
        ...
        const handleScroll = () => {
            try {
                window.sessionStorage.setItem(SIDEBAR_SCROLL_TOP_KEY, String(element.scrollTop));
            } catch {
    ...
    return (
        <div
            ...
            className={cn('sidebar-scroll flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden group-data-[collapsible=icon]:overflow-hidden', className)}
            {...props}
        />
    );
});
